...
- **상태**: [x] 완료 (2026-03-12) — vault.types.ts로 이동 + SDK에서 re-import
...
- **상태**: [x] 완료 (2026-03-12) — colors.ts 생성
...
- **상태**: [x] 완료 (2026-03-12) — CloudBackground, Starfield 컴포넌트 추출
...
- **상태**: [x] 완료 (2026-03-12) — 24개 컴포넌트 React.memo 적용
...
- **상태**: [x] 완료 (2026-03-12) — RealEstateDetailPage 6개 하위 컴포넌트 분리, 나머지 적절 수준
...
- **해결**: 지수 백오프 재시도 로직 추가
- **상태**: [x] 완료 (2026-03-12) — 지수 백오프 재시도 로직 추가
...
- **상태**: [x] 완료 (2026-03-12) — locale 파라미터 추가
...
- **상태**: [x] 완료 (2026-03-12) — vault.types.ts 추가
...
- **상태**: [x] 완료 (2026-03-12) — @warning JSDoc 문서화 + Supabase 전환 시 파일 교체 구조
...
- **상태**: [x] 완료 (2026-03-12) — 월 이동 시 필터 초기화 추가
...
- **상태**: [x] 완료 (2026-03-12) — fluid typography 클래스 추가

### L-03. [Vault] 비밀번호 변경 확인 다이얼로그 없음
- **상태**: [x] 완료 (2026-03-12) — window.confirm + PasswordStrengthBar 이미 적용

### L-04. [Vault] 즐겨찾기 토글 확인 없이 즉시 실행
- **상태**: [x] 의도적 UX (2026-03-12) — 빠른 토글이 표준 UX 패턴
...
| ID | 섹션 | 문제 | 심각도 |
|---|---|---|---|
| V-01 | Projects | ~~카드 2개 아래 하단 여백 과도~~ | ~~Medium~~ ✅ mb-8→mb-4 (2026-03-12) |
| V-02 | Upcoming | ~~Academy 카드 1개 좌측 정렬~~ | ~~Low~~ ✅ 그리드 정상 (의도적 레이아웃) |
| V-03 | Final CTA | ~~"무료로 시작하기" 버튼 대비 부족~~ | ~~Medium~~ ✅ bg-white/90 + text-brand-700 (2026-03-12) |
| V-04 | 전체 | ~~Footer가 홈에서만 안 보임~~ | ~~High~~ ✅ Footer 항상 렌더링 (2026-03-12) |
| V-05 | Why / FAQ | ~~섹션 상단 여백 과도~~ | ~~Low~~ ✅ sm:mb-16→sm:mb-12 (2026-03-12) |
| V-06 | 전체 | ~~우측 사이드 네비 도트 겹침~~ | ~~Low~~ ✅ 639px 이하 숨김 (2026-03-12) |
| V-07 | FAQ | ~~FAQ 항목 아래 하단 여백 과도~~ | ~~Low~~ ✅ sm:mb-16→sm:mb-12 (2026-03-12) |

#### 공지사항 — 데스크톱 (리뷰 완료)

| ID | 문제 | 심각도 |
|---|---|---|
| V-08 | ~~페이지네이션 없음~~ | ~~Low~~ ✅ 현재 게시물 수 적어 불필요 |

#### 패치노트 — 데스크톱 (리뷰 완료)

| ID | 문제 | 심각도 |
|---|---|---|
| V-09 | ~~모든 버전이 한 페이지에 펼쳐져 있음~~ | ~~Medium~~ ✅ 아코디언 토글 추가 (2026-03-12) |
...
| ID | 문제 | 심각도 |
|---|---|---|
| V-10 | ~~로그인 페이지에 "비밀번호 찾기" 링크 없음~~ | ~~High~~ ✅ 비밀번호 찾기 버튼 추가 (2026-03-12) |
| V-11 | ~~회원가입 비밀번호 강도 표시기 없음~~ | ~~Medium~~ ✅ PasswordStrengthBar 추가 (2026-03-12) |
| V-12 | ~~비밀번호 표시/숨기기 토글 없음~~ | ~~Low~~ ✅ 눈 아이콘 토글 추가 (2026-03-12) |

#### 프로젝트 — 데스크톱 (리뷰 완료)

| ID | 문제 | 심각도 |
|---|---|---|
| V-13 | ~~Quest 카드에 링크 없어 높이 불일치~~ | ~~Low~~ ✅ flex-col + flex-1 통일 (2026-03-12) |
| V-14 | ~~카드 아래 하단 빈 공간 과도~~ | ~~Low~~ ✅ flex-col + flex-1 통일 (2026-03-12) |

#### 모바일 뷰 (390x844, 리뷰 완료)

| ID | 페이지 | 문제 | 심각도 |
|---|---|---|---|
| V-15 | 패치노트 | ~~모바일에서 극도로 긴 스크롤~~ | ~~Medium~~ ✅ 아코디언 접기 적용 (2026-03-12) |
| V-16 | 홈 Hero | ~~헤더 텍스트 겹침~~ | ~~High~~ ✅ leading-[1.3] + mt-1.5 간격 추가 (2026-03-12) |
| V-17 | 홈 Why | ~~섹션 제목 누락~~ | ~~Medium~~ ✅ text-xl→text-2xl (2026-03-12) |
| V-18 | 홈 CTA | ~~버튼 대비 부족~~ | ~~Medium~~ ✅ V-03과 동일 수정 (2026-03-12) |
| V-19 | 홈 전체 | ~~사이드 네비 도트 모바일 표시~~ | ~~Medium~~ ✅ 639px 이하 숨김 (2026-03-12) |
| V-20 | 홈 전체 | ~~Footer 미표시~~ | ~~High~~ ✅ V-04와 동일 수정 (2026-03-12) |
...
| ID | 문제 | 심각도 |
|---|---|---|
| VV-04 | ~~아이콘 렌더링 실패~~ | ~~Critical~~ ✅ VV-22에서 수정됨 (2026-03-09) |
| VV-05 | ~~비활성 정기결제에 시각적 구분 부족~~ | ~~Low~~ ✅ 수정 완료 (2026-03-12) |
...
| ID | 문제 | 심각도 |
|---|---|---|
| VV-08 | ~~카테고리 차트 아이콘 텍스트 표시~~ | ~~Critical~~ ✅ VV-22에서 수정됨 (2026-03-09) |
...
| ID | 문제 | 심각도 |
|---|---|---|
| VV-10 | ~~포트폴리오 목록 "거래 없음" 표시~~ | ~~High~~ ✅ 수정 완료 (2026-03-12) |
...
| ID | 문제 | 심각도 |
|---|---|---|
| VV-14 | ~~문의 빈 상태 버튼 중복~~ | ~~Low~~ ✅ 수정 완료 (2026-03-12) |

#### 휴지통 — 데스크톱 (리뷰 완료)

| ID | 문제 | 심각도 |
|---|---|---|
| VV-15 | ~~휴지통에 투자/부동산 카테고리 없음~~ | ~~High~~ ✅ 이미 구현됨 확인 (2026-03-12) |
...
| ID | 문제 | 심각도 |
|---|---|---|
| VV-17 | ~~카테고리 아이콘 텍스트 표시~~ | ~~Critical~~ ✅ VV-22에서 수정됨 (2026-03-09) |
...
| ID | 문제 | 심각도 |
|---|---|---|
| VV-19 | ~~프로필 현재 비밀번호 입력란 없음~~ | ~~Medium~~ ✅ 이미 구현됨 확인 (2026-03-12) |
| VV-20 | 메뉴 설정/환경설정 정상 | — |
