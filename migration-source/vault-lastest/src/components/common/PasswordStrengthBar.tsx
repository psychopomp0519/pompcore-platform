/**
 * @file PasswordStrengthBar.tsx
 * @description 비밀번호 강도 막대 + 규칙 체크리스트 컴포넌트
      ...
      <ul className="space-y-1">
        {result.rules.map((rule) => (
          <li key={rule.id} className="flex items-center gap-1.5 text-xs">
            {rule.passed ? (
              <svg className="h-3.5 w-3.5 flex-shrink-0 text-vault-color" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5} aria-hidden="true">
